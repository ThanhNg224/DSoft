...
class DebtPaidScreen extends BaseView<DebtPaidController> {
  final DebtManagementState state;
  const DebtPaidScreen({super.key, required this.state});

  @override
  DebtPaidController createController(BuildContext context)
  => DebtPaidController(context);
  ...
  Widget _body() {
    if(controller.screenStateIsLoading) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Utilities.retryButton(() => controller.onGetListPaid()),
      ...
      if(state.listDebtPaid.isEmpty) {
        return WidgetListView(
            onRefresh: () async => controller.onGetListPaid(),
            children: [Utilities.listEmpty()]
        );
      }
      return WidgetListView.builder(
        itemCount: state.listDebtPaid.length,
        onRefresh: () async => controller.onGetListPaid(isLoad: false),
        itemBuilder: (context, index) {
          bool isUnfinished = state.listDebtPaid[index].status == 0;

          return GestureDetector(
            onTap: () => Navigator.pushNamed(context, DebtAddCollectionScreen.router, arguments: DebtManagementSend(
              data: state.listDebtPaid[index],
              isCollectionDebt: false
            )),
            child: Container(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            state.listDebtPaid[index].fullName ?? "Đang cập nhật",
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            state.listDebtPaid[index].staff?.name ?? "Đang cập nhật",
                            style: TextStyles.def.size(15).colors(MyColor.slateGray).bold,
                            overflow: TextOverflow.ellipsis,
                          ),
                        ),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            "Số tiền: ${state.listDebtPaid[index].total?.toCurrency() ?? "Đang cập nhật"}",
                            style: TextStyles.def.size(15).colors(MyColor.slateGray).bold,
                            overflow: TextOverflow.ellipsis,
                          ),
                        ),
